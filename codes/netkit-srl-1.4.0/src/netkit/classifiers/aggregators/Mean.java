 ...
 */
public final class Mean extends AggregatorImp {
    public Mean(EdgeType edgeType, Attribute attribute) {
        super("mean",edgeType, attribute, Type.CONTINUOUS);
        if(!((attribute instanceof AttributeDiscrete) || (attribute instanceof AttributeContinuous)))
            throw new IllegalArgumentException("Mean["+getName()+"] aggregator can only be used with numerical attributes.");
    }

    public double getValue(Node n, Estimate prior) {
        return getNodeInfo(n).getMean(n);
    }
}
