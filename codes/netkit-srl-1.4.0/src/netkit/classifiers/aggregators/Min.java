 ...
 */
public final class Min extends AggregatorImp {
    public Min(EdgeType edgeType, Attribute attribute) {
        super("min",edgeType, attribute, Type.CONTINUOUS);
        if(!((attribute instanceof AttributeDiscrete) || (attribute instanceof AttributeContinuous)))
            throw new IllegalArgumentException("Min["+getName()+"] aggregator can only be used with numerical attributes.");
    }

    public double getValue(Node n, Estimate prior) {
        return getNodeInfo(n).getMin(n);
    }
}
