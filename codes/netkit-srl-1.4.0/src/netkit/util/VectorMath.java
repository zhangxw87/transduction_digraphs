...
/**
 * $Id: VectorMath.java,v 1.5 2007/03/26 23:45:07 sofmac Exp $
 * Part of the open-source Network Learning Toolkit
 *
 * User: smacskassy
 * Date: Dec 1, 2004
...
public class VectorMath {
  ...
  public static double dotproduct(double[] arr1, double[] arr2) {
    ...
    if(arr1 == null || arr2 == null || arr1.length != arr2.length)
      throw new ArithmeticException("cannot calculate dotproduct two arrays of different sizes arr1="+((arr1==null)?-1:arr1.length)+" arr2="+((arr2==null)?-1:arr2.length));
    double value = 0;
  ...
  public static double[] merge(double alpha, double[] arr1, double[] arr2) {
    if(arr1 == null && arr2 == null)
      return null;
    if(arr1 == null || arr2 == null || arr1.length != arr2.length)
      throw new ArithmeticException("cannot merge two arrays of different sizes arr1="+((arr1==null)?-1:arr1.length)+" arr2="+((arr2==null)?-1:arr2.length));
    if(alpha<0 || alpha>1)
      throw new ArithmeticException("invalid alpha="+alpha+" (must lie in the range 0-1 inclusive)");
  ...
  public static void merge(double alpha, double[] arr1, double[] arr2, double[] result) {
    ...
    if(arr1 == null || arr2 == null || arr1.length != arr2.length)
      throw new ArithmeticException("cannot merge two arrays of different sizes arr1="+((arr1==null)?-1:arr1.length)+" arr2="+((arr2==null)?-1:arr2.length));
    if(result == null || result.length < arr2.length)
      throw new ArithmeticException("result array length="+((result==null)?-1:result.length)+" is too small.  need to have length="+((arr2==null)?-1:arr2.length));
    if(alpha<0 || alpha>1)
      throw new ArithmeticException("invalid alpha="+alpha+" (must lie in the range 0-1 inclusive)");
    if(alpha==1)
  ...
  public static double l2_length(double[] e) {
    ...
    for (double d : e)
      tot += d*d;
    return (tot==0 ? 0 : Math.sqrt(tot));
  }

  public static void l2_normalize(double[] e) {
    double len = l2_length(e);
    if(len == 0)
    {
      len = Math.sqrt(1.0/(double)e.length);
  ...
  public static void add(double[] arr1, double[] arr2) {
    if(arr1 == null || arr2 == null || arr1.length != arr2.length)
      throw new IllegalArgumentException("Arrays1 and 2 are not of same length!");
    for(int i=0;i<arr1.length;i++)
      arr1[i] += arr2[i];
  }
  ...
  public static <T> void randomize(T[] src, T[] dst, int numItems) {
    if(src == null)
      return;
    if(numItems > dst.length || numItems > src.length)
      numItems = Math.min(dst.length, src.length); 
    if(src!=dst)
