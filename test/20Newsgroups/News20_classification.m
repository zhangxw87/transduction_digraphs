...
fid = fopen('News20_edge.rn','w');
fprintf(fid,'%d,%d,1\n',edge_mat');
fclose(fid);

node_mat = [paper_id; class_label_number];
fid = fopen('News20_truth.csv','w');
fprintf(fid,'%d,%d\n',node_mat);
fclose(fid);
...
for i=1:Rept
    for j=1:ratio_len
        ...
        fid = fopen('News20_temp_label_file.csv','w');
        fprintf(fid,'%d,%d\n',temp_label);
        fclose(fid);
        ...
        fprintf('WVRN...\n');
        t = tic;
        [sta,cmdout]=system('java -jar ../../codes/netkit-srl-1.4.0/lib/NetKit-1.4.0.jar -output output-file -pruneZeroKnowledge -known News20_temp_label_file.csv -lclassifier uniform -rclassifier wvrn -inferencemethod relaxlabel -showauc News20_schema.arff > News20_aucfile.out');
        WVRN_Result.time(i,j) = toc(t);
        fid = fopen('News20_aucfile.out','r');
        sum=0;
        for k=1:NumClass
        ...
        fprintf('CDRN...\n');
        t = tic;
        [sta,cmdout]=system('java -jar ../../codes/netkit-srl-1.4.0/lib/NetKit-1.4.0.jar -output output-file -pruneZeroKnowledge -known News20_temp_label_file.csv -lclassifier uniform -rclassifier cdrn-norm-cos -inferencemethod relaxlabel -showauc News20_schema.arff > News20_aucfile.out');
        ...
        fprintf('NBC...\n');
        t = tic;
        [sta,cmdout]=system('java -jar ../../codes/netkit-srl-1.4.0/lib/NetKit-1.4.0.jar -output output-file -pruneZeroKnowledge -known News20_temp_label_file.csv -lclassifier uniform -rclassifier nobayes -inferencemethod relaxlabel -showauc News20_schema.arff > News20_aucfile.out');
        ...
        fprintf('NLB...\n');
        t = tic;
        [sta,cmdout]=system('java -jar ../../codes/netkit-srl-1.4.0/lib/NetKit-1.4.0.jar -output output-file -pruneZeroKnowledge -known News20_temp_label_file.csv -lclassifier uniform -rclassifier nolb-lr-count -inferencemethod iterative -showauc News20_schema.arff > News20_aucfile.out');
...
save ../../results/News20_result.mat -append WVRN_Result ...
