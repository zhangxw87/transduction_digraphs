...
save ../../results/TDT2_result.mat samples DiGraphARW_Result SGL_Result CTK_Result ...
...
fid = fopen('TDT2_edge.rn','w');
fprintf(fid,'%d,%d,1\n',edge_mat');
fclose(fid);

node_mat = [paper_id; class_label_number];
fid = fopen('TDT2_truth.csv','w');
fprintf(fid,'%d,%d\n',node_mat);
fclose(fid);
...
for j=1:Rept
    for i=1:ratio_len
        ...
        fid = fopen('TDT2_temp_label_file.csv','w');
        fprintf(fid,'%d,%d\n',temp_label);
        fclose(fid);
        
        % Weighted Vote Relational Neighbor Classifier
        t = tic;
        [sta,cmdout]=system('java -jar ../../codes/netkit-srl-1.4.0/lib/NetKit-1.4.0.jar -output output-file -pruneZeroKnowledge -known TDT2_temp_label_file.csv -lclassifier uniform -rclassifier wvrn -inferencemethod relaxlabel -showauc TDT2_schema.arff > TDT2_aucfile.out');
        WVRN_Result.time(i,j) = toc(t);
        fid = fopen('TDT2_aucfile.out','r');
        sum=0;
        for k=1:NumClass
        ...
        t = tic;
        [sta,cmdout]=system('java -jar ../../codes/netkit-srl-1.4.0/lib/NetKit-1.4.0.jar -output output-file -pruneZeroKnowledge -known TDT2_temp_label_file.csv -lclassifier uniform -rclassifier cdrn-norm-cos -inferencemethod relaxlabel -showauc TDT2_schema.arff > TDT2_aucfile.out');
        ...
        t = tic;
        [sta,cmdout]=system('java -jar ../../codes/netkit-srl-1.4.0/lib/NetKit-1.4.0.jar -output output-file -pruneZeroKnowledge -known TDT2_temp_label_file.csv -lclassifier uniform -rclassifier nobayes -inferencemethod relaxlabel -showauc TDT2_schema.arff > TDT2_aucfile.out');
...
save ../../results/TDT2_result.mat -append WVRN_Result CDRN_Result NBC_Result;
